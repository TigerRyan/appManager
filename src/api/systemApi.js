...
// 保存配置
export const putConfigApi2 = ({key, val, status, type} = {}) => {
  if(type >= 3){
    let num = status ? val : 999999999
    return ajax.post(`${ROOT}/put/${key}`, num)
  }else{
    return ajax.post(`${ROOT}/put/${key}`, {status,val})
  }
}
// 服务商签约周期
export const manageApi =(val) => ajax.put('/systemManageApi/updateValid',val)
// 设置服务商分成比例
export const manageUpdateApi =(val) => ajax.put('/systemManageApi/updateRate',val)
// 获取服务商分成比例
export const getManageApi =() => ajax.get('/systemManageApi/getRate')
// 获取服务商签约周期
export const getManageUpdateApi = () => ajax.get('/systemManageApi/getValid')
