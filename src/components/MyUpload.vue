...
<template>
  <div class="my_upload" :class="{hide:list.length >= length}">
    ...
    <el-dialog v-model="showPic" :modalAppendToBody="false" title="预览" width="600px">
  ...
  export default {
    ...
    created() {
      ...
      if (myUploadTag === null) {
        ...
        myUploadTag.innerHTML = `.my_upload .el-upload-list__item,.my_upload .el-upload--picture-card{width:${this.width}px;height:${this.height}px;line-height:${this.height}px;}.my_upload .el-icon-upload-success.el-icon-check{right:${this.height / 10}px}`
