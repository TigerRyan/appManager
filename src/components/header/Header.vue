...
<template>
  <div class="header header_dialog">
    <el-dropdown class="user" @command="handleCommand" trigger="click">
      <span class="el-dropdown-link">
        <img :src="mine.avatar || '/static/img/admin.png'" @error="mine.avatar = '/static/img/admin.png'">
        <!--<span>{{mine.name || mine.nickName || '管理员'}}</span>-->
        <i class="el-icon-arrow-down el-icon--right"></i>
      </span>
      <el-dropdown-menu slot="dropdown" class="header-dropdown">
        <el-dropdown-item command="info"><i class="el-icon-my-peoplefill"/>我的资料</el-dropdown-item>
        <el-dropdown-item command="password"><i class="el-icon-setting"/>修改密码</el-dropdown-item>
        <el-dropdown-item command="logout"><i class="el-icon-my-logout"/>退出登录</el-dropdown-item>
      </el-dropdown-menu>
    </el-dropdown>
    <el-dialog v-if="dialog.data.id" :modalAppendToBody="false" :title="dialog.title" :visible.sync="dialog.show" top="20px" width="600px" customClass="my-dialog">
      <el-form :model="dialog.data" ref="form1" labelWidth="160px">
        <el-form-item prop="avatar" labelWidth="0" class="el-form-zl">
          <Avatar :url="dialog.data.avatar" @success="dialog.data.avatar = $event"/>
        </el-form-item>
    ...
    <el-dialog v-if="!dialog.data.id" :modalAppendToBody="false" :title="dialog.title" :visible.sync="dialog.show" width="450px">
      <el-form :model="dialog.data" :rules="rule2" ref="form2" labelWidth="120px">
...
<script>
  import {mineApi, logoutApi, changePasswordApi, editAdminApi} from '../../api/adminApi'
