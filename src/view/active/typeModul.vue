...
import { detailsApi,deletDatatApi,sortParticularIdsApi } from "../../api/activity";
export default {
  name: "modulData",
  ...
  methods: {
    ...
    sortIds(){
      sortParticularIdsApi({particularId:this.data.particularsId,ids:this.data.ids}).then(data=>{
        console.log(data,'ids....')
         this.findList()
        })
    },
    add() {
      this.$router.push({ path: "imgModul" ,query:{dataType:this.dataType,id:this.id}});
    },
    yichu(val){  
      deletDatatApi({dataId:val.id,id:this.id}).then(data=>{
           this.$message({
            message: "删除成功",
            type: "success"
          });
