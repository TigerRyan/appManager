<!-- 已结算详情 -->
<template>
  <div class='content_data balanceVideoDetail'>
     <Breadcrumb/>
     ...
     <div class="courseinf">
        ...
        <div class="courid">
            <ul class="ul_one">
               ...
               <li>
                  ...
                  <span class="fourword">{{data.gradeName}}</span>
               </li>
               <li>
                  <span class="span_one">教学科目</span>
                  <!-- <span class="fourword" v-for="(item,index) in showsubjects" :key="index">{{item}}</span> -->
                  <span class="fourword">{{data.subjectName}}</span>
               </li>
               <li>
                  <span class="span_one">授课教师</span>
                  <span class="spanimg"><img :src="data.teacherPic" alt=""></span>
        ...
        <div class="courtime">
           <ul class="ul_one">
               <li>
                  <span class="span_one">总章节</span>
                  <span class="threeword">{{data.allChapter}}</span>
               </li>
               <li>
     ...
     <!-- 列表 -->
      <div class="list_content">
          <ul class="list_title">
            ...
            <li>第三方订单号</li>
            <li>支付时间</li>
            <li v-if="type == 2">教师视频分成比例</li>
            <li v-if="type == 2">教师分润</li>
            <li v-if="type == 1">服务商分成比例</li>
            <li v-if="type == 1">服务商分润</li>
            <li v-if="type == 1">机构分成比例</li>
            <li v-if="type == 1">机构分润</li>
          </ul>
          <div class="list" v-for="(item,index) in list" :key="index">
            <p class="number_time">
               <span class="p_span">订单号:{{item.orderNo}}</span>
               <span>下单时间:{{dateTimeFilter(0,0,item.payTime)}}</span>
               <span class="settle">结算时间：{{datenTimeFilter(0,0,item.settlementTime)}}</span>
            </p>
            <ul class="list_wrap">
              <li class="need_flex">
              ...
              <li>{{defValFilter01(0,0,item.thirdOrderNo)}}</li>
              <li>{{datenTimeFilter(0,0,item.payTime)}}</li>
              <li v-if="type == 2">{{item.serviceRate}}</li>
              <li v-if="type == 2">￥{{currencyFilter(0,0,item.serviceMoney)}}</li>
              <li v-if="type == 1">{{item.serviceRate}}</li>
              <li v-if="type == 1">￥{{currencyFilter(0,0,item.serviceMoney)}}</li>
              <li v-if="type == 1">{{item.mechanismRate}}</li>
              <li v-if="type == 1">￥{{currencyFilter(0,0,item.mechanismMoney)}}</li>
            </ul> 
          </div> 
      </div>
...
import { getVideoSpecialApi,getVideoDoneApi,getServeCourceApi } from '../../../src/api/balanceListApi'
import {payTypeFilter3,datenTimeFilter,dateTimeFilter,datesTimeFilter,datetTimeFilter,currencyFilter,defValFilter01} from '../../constant/filter'
export default {
  name: 'balanceVideoDetail',
  ...
  data () {
    return {
      ...
      type:null,
      id:null,
      navTitle:["学员","实付金额/元","第三方订单号","支付时间","教师视频分成比例","教师分润","服务商分成比例","服务商分润","机构分成比例","机构分润"],
       pageOption:{
         page:1,
         pageSize:10,
         total:null,
  ...
  methods: {
    ...
    defValFilter01,
    findList(page={}){
      getVideoDoneApi({...this.pageOption,...page,courseId:this.id,courseType:2,type:this.type})
        .then(data=>{
     ...
     init(){
       getVideoSpecialApi(this.id).then(data=>{
