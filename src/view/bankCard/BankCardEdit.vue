...
<script>
  import {getAdminApi, createAdminApi, updateAdminApi} from '../../../api/adminApi'
      ...
      submit() {
        this.$refs.admin.validate((valid) => {
          if (valid) {
            ...
            this.admin.roles = this.admin.roles.map(id => ({id}))
            const fn = id === CREATE ? createAdminApi(this.admin) : updateAdminApi(this.admin)
            fn.then(() => {
