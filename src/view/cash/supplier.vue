<template>
	<div class="content_data cashSupplier">
        <Breadcrumb/>
    <div>
	...
	<TablePage v-if='data.status==1' key='1'>
      <el-table :data="data.list"  @selection-change="multipleSelect" v-loading="data.loading"  ref="multipleTable" @sort-change="sortChange" >
        ...
        <el-table-column prop="fullName" label="提现服务商"  min-width="200">
          <template slot-scope="scope">
            <!-- <img :src="scope.row.logo || '/static/img/admin.png'" width="80" height="80" style='vertical-align: middle;'/> -->
	...
	<TablePage v-if='data.status==2' key='2'>
      <el-table :data="data.list" v-loading="data.loading" @sort-change="sortChange"  ref="multipleTable2">
        ...
        <el-table-column prop="operation" label="操作"  min-width="150" fixed='right'>
          <template slot-scope="scope">
	...
	<TablePage v-if='data.status==3' key='3'>
      <el-table :data="data.list" v-loading="data.loading" @sort-change="sortChange"  ref="multipleTable4">
        ...
        <el-table-column prop="reason" class='operation'  label="拒绝理由" mix-width="200"  />
      </el-table>
	</TablePage>
    <!-- 分页 -->
    ...
    <!-- 确认打款弹框-->
    <el-dialog title="请输入银行转账交易单号" @close="closeDia2" :visible.sync="dialog2.show" :modal-append-to-body="false" top='30vh'>
      <el-form :model="dialog2" :rules="rules" ref="menu">
          <el-form-item prop="serialNumber">
              <el-input v-model.trim ="dialog2.serialNumber" type="text"></el-input>
          </el-form-item>
      </el-form>
...
import {getSupplierCashListApi,auditSupplierCashApi,affirmSupplierCashApi} from '../../api/supplierApi'
import {exportProviderCashListApi} from '../../api/appCashApi'
export default {
  name: "cashSupplier",
  components: {
    MyPagination,
  ...
  data() {
    return {
        value: null,//搜索类型
        placeholder:'请输入服务商名称',//搜索框默认文字
        list: [{value:1, label: '服务商名称'}, {value:2, label: '手机号'}, {value:3, label: '提现单号'}],
      // 大于当前时间不可选
        pickerOptions0: {
          disabledDate(time) {
  ...
  methods: {
    ...
    auditFilter2,
      findList(page = {}) {
        ...
        getSupplierCashListApi({...this.data, ...page}).then((data) => {
          this.data = {...this.data, ...page, ...data, loading: false}
        })
      },
    changeSelect(){
      this.data.keyType = this.value
      this.placeholder = ['请输入服务商名称','请输入手机号','请输入提现单号'][this.value-1]
    },
    //4个tab切换
    redBill(id) {
        this.data.status = id
    ...
    //审核通过
      agree(row,status,rows){
        ...
        .then(() => {
          auditSupplierCashApi({ids:[row.id],status}).then(()=>{
            this.findList()
    ...
    //拒绝
      check() {
        auditSupplierCashApi({ ids:[this.dialog.id],status:this.dialog.status,reason:this.dialog.reason}).then(()=>{
    ...
    exportCashList({start, end}) {
      return exportProviderCashListApi({ ...this.data,start, end})
    },
    //批量通过
    batchAgree(){
        if(this.multipleSelectId.length){
          ...
          {confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning', center: true})
          .then(() => {
            auditSupplierCashApi({ids:this.multipleSelectId,status:2}).then(()=>{
              this.findList()
              this.$message({type: 'success', message: '确认通过!'})})
            })
          .catch(() => {
    ...
    //批量拒绝
    batchCheck(){
      auditSupplierCashApi({ ids:this.multipleSelectId,status:3,reason:this.dialog3.reason}).then(()=>{
    ...
    affirm(){
      this.$refs.menu.validate(valid =>{
        if(valid){
          affirmSupplierCashApi(this.dialog2.id,this.dialog2.serialNumber).then(()=>{
        this.findList()
