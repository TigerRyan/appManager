    ...
    <!-- 列表 -->
		<TablePage>
          ...
          <el-table-column type="selection" width="55" align="center"> </el-table-column>
					<el-table-column prop="content" label="评论内容" min-width="130" align="center">
              <template slot-scope="scope">
                <el-tooltip placement="top">
                  ...
                  <span>{{  scope.row.content.length>=20?scope.row.content.slice(0,20)+'...':scope.row.content  }}</span>
					...
					<el-table-column prop="user" label="用户" min-width="110" align="center">
						<template slot-scope="scope">
						 <img :src="scope.row.pic || '../../../static/img/logo.png'" width="70"/>
             <span>{{ scope.row.nikename }}</span>
					...
					<el-table-column  label="操作" class=""  min-width="200" align="center">
            <template slot-scope="scope">
...
import { getCommentListApi,getDelApi } from "../../api/reviewComments"
export default {
  name: 'reviewComments',
    ...
    return {
      ...
      id:JSON.parse(this.$route.query.id),
        data:{
          ...
          pageSize:10,
          list:[
            {content:'这是评论内容这是评论内容这是评论内容这是评论内容这是评论内容这是评论内容这是评论内容这是评论内容这是评论内容这是评论内容这是评论内容这是评论内容',user:'陈双林',commentTime:''},
            {content:'这是评论内容这是评论内容这是评论内容这是评论内容这是评论内容这是评论内容这是评论内容这是评论内容这是评论内容这是评论内容这是评论内容这是评论内容',user:'张三',commentTime:''}
          ]
        },
        multipleSelection: []
    }
  ...
  methods: {
    ...
    findList(page = {}){
      getCommentListApi({infoId:this.id,...page}).then(data=>{
    ...
    toggleSelection(rows) {
      ...
      delet(val){
        ...
        if (val=='all' && this.multipleSelection.length <= 0) {
          ...
          getDelApi({id:ids.join(',')}).then(data=>{
            this.$message({
              message: '批量删除成功',
              type: 'success'
            });
