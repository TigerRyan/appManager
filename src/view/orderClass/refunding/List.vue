<template>
  <div class="content_data">
    <Breadcrumb/>
    <TablePage>
      <el-input placeholder="输入购买者姓名或手机号码或第三方支付流水号或课程ID查询..." style="width: 450px" suffix-icon="el-icon-search" v-model="data.name" @blur="search"/>
      <el-table :data="data.list" v-loading="data.loading" element-loading-text="拼命加载中...">
        ...
        <el-table-column prop="name" label="课程名称" :formatter="defValFilter02" min-width="100"/>
        ...
        <el-table-column prop="salePrice" label="课程优惠价格" :formatter="currencyFilter" min-width="110"/>
        <el-table-column prop="orderNumber" label="APP订单编号" :formatter="defValFilter02" min-width="160"/>
        <el-table-column prop="thirdNumber" label="第三方支付流水号" :formatter="defValFilter02" min-width="160"/>
        ...
        <el-table-column prop="reason" label="退款原因" :formatter="defValFilter02" min-width="200"/>
        <el-table-column prop="studentName" label="学生姓名" :formatter="defValFilter02" min-width="100"/>
        <el-table-column prop="studentPhone" label="学生手机号码" :formatter="defValFilter02" min-width="110"/>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        data: {page: 1, pageSize: 10, list: [], total: 0, status: 5, loading: false, name: null},
      }
    },
    methods: {
      dateTimeFilter,
