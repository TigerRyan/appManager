<template>
  <div class="content_data">
    <Breadcrumb/>
    <TablePage>
      <el-input placeholder="输入课程名称或教师名称或教师手机号码或课程ID..." style="width: 450px" suffix-icon="el-icon-search" v-model="data.keyWord" @blur="search"/>
      <el-table :data="data.list" v-loading="data.loading" element-loading-text="拼命加载中...">
        ...
        <el-table-column prop="totalPrice" label="课程总价" :formatter="currencyFilter" min-width="100"/>
        ...
        <el-table-column prop="userName" label="教师姓名" :formatter="defValFilter02" min-width="120"/>
        <el-table-column prop="userPhone" label="教师手机号码" :formatter="defValFilter02" min-width="120"/>
...
<script>
  ...
  import {dateTimeFilter, defValFilter02, currencyFilter, courseGradeFilter, hourFilter} from '../../../constant/filter'
  ...
  export default {
    ...
    data() {
      return {
        data: {page: 1, pageSize: 10, list: [], total: 0, loading: false, keyWord: null, courseStatus: 0},
      }
    },
    methods: {
      dateTimeFilter,
      ...
      findList(page = {}) {
        ...
        getEduItemApi().then(() => getCourseApi({...this.data, ...page}).then((data) => {
