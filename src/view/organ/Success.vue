<template>
  <div class="organSuccess content_data">
    ...
    <div class="search kind">
      ...
      <el-input v-model.trim="data.query" clearable @keydown.enter.native="search" @blur="search" placeholder="请输入机构名称或任务ID号查询" style="width: 276px;"></el-input>
    ...
    <TablePage>
      <el-table :data="data.list" v-loading="data.loading" element-loading-text="拼命加载中..." ref="multipleTable">
        ...
        <el-table-column prop="createTime" label="注册时间" :formatter="detailTimeFilter" width="180" />
        <el-table-column prop="passTime" label="审核通过时间" :formatter="detailTimeFilter" width="180" />
        <el-table-column prop="statue" label="状态" width="90">
          <template slot-scope="scope">
            <span :style="{color: scope.row.statue ? '#909399' : '#409EFF' }"> {{statueFilter(0,0, scope.row.statue)}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="isRecommend" label="推荐状态" width="120">
          <template slot-scope="scope">
            <span :style="{color: scope.row.isRecommend ? '#F56c6c' : '#67c23A' }"> {{isRecommendFilter(0,0, scope.row.isRecommend)}}</span>
          </template>
        </el-table-column>
        <el-table-column label="操作" min-width="180">
          <template slot-scope="scope">
            <el-button v-show="false" type="edit" size="middle" @click="passHandele(scope.row.id)" :disabled='disabled'>编辑</el-button>
            <el-button plain type="primary" size="middle" @click="$router.push({path:'mechanismDetail',query:{id:scope.row.id}})">查看</el-button>
            <el-button plain v-if="!scope.row.isRecommend" type="danger" size="middle" @click="cancleHandle(scope.row)" :disabled='disabled'>取消推荐</el-button>
            <el-button plain v-if="scope.row.isRecommend" type="success" size="middle" @click="setHanddle(scope.row)" :disabled='disabled'>设置推荐</el-button>
          </template>
        </el-table-column>
      </el-table>
    </TablePage>
    <el-dialog :title="menu.tip" width="30%" :modal="true" customClass="my-dialog" @close="clearValidate" :visible.sync="menu.show" :append-to-body="true">
...
export default {
  ...
  methods: {
    ...
    exportList({ start, end }) {
      return exportMechanismsApi({ start, end })
    },
    findList(page = {}) {
