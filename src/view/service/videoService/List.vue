  ...
  <div class="content_data">
    ...
    <TablePage>
      <el-table :data="data.list" v-loading="data.loading" element-loading-text="拼命加载中...">
        ...
        <!--<el-table-column prop="icon" label="服务图片" min-width="200">-->
        <!--<template slot-scope="scope">-->
        <!--<ImgView :src="scope.row.icon" width="180" height="90"/>-->
        <!--</template>-->
        <!--</el-table-column>-->
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        data: {},
        dialog: {show: false, type: 2, id: null, name: '', icon: null, weight: null},
        rules: {
    ...
    methods: {
      ...
      del({id, name}) {
        confirm(`确认删除视频课程特色标签“${name}”?`, 'error', '删除确认?').then(_ => delLabelsApi(id).then(_ => {
          success(`删除视频课程特色标签“${name}”成功！`)
          this.findList()
        }).catch(({msg}) => error(msg || '操作失败！请稍后重试...')))
      },
      edit(data = {}) {
        if (data.id) {
          this.dialog = {...this.dialog, ...data, title: `修改视频课程特色标签“${data.name}”`, show: true}
        } else {
          this.dialog = {...this.dialog, title: '添加视频课程特色标签', show: true, id: null, name: '', icon: null, weight: null}
        }
      },
      submit() {
        this.$refs.dialog.validate(valid => {
          if (valid) {
            ...
            fn.then(_ => {
              success(`${this.dialog.id ? '修改' : '添加'}视频课程特色标签“${this.dialog.name}”成功！`)
              this.dialog.show = false
              this.findList()
            }).catch(({msg}) => error(msg || `${this.dialog.id ? '修改' : '添加'}视频课程特色标签“${this.dialog.name}”失败！请稍后重试...`))
          } else {
            return false
          }
        })
