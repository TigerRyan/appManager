...
<script>
  ...
  export default {
    ...
    data() {
      return {
        data: {},
        dialog: {show: false, type: 4, id: null, name: '', icon: null, weight: null},
        rules: {
    ...
    methods: {
      ...
      del({id, name}) {
        confirm(`确认删除教学特色标签“${name}”?`, 'error', '删除确认?').then(_ => delLabelsApi(id).then(_ => {
          success(`删除教学特色标签“${name}”成功！`)
          this.findList()
        }).catch(({msg}) => error(msg || '操作失败！请稍后重试...')))
      },
      edit(data = {}) {
        if (data.id) {
          this.dialog = {...this.dialog, ...data, title: `修改教学特色标签“${data.name}”`, show: true}
        } else {
          this.dialog = {...this.dialog, title: '添加教学特色标签', show: true, id: null, name: '', icon: null, weight: null}
        }
      },
      submit() {
        this.$refs.dialog.validate(valid => {
          if (valid) {
            ...
            fn.then(_ => {
              success(`${this.dialog.id ? '修改' : '添加'}教学特色标签“${this.dialog.name}”成功！`)
              this.dialog.show = false
              this.findList()
            }).catch(({msg}) => error(msg || `${this.dialog.id ? '修改' : '添加'}教学特色标签“${this.dialog.name}”失败！请稍后重试...`))
          } else {
            return false
          }
        })
