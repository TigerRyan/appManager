    ...
    <el-form :model="admin" :rules="rules" ref="admin" labelWidth="200px">
      ...
      <el-form-item label="角色权限：" prop="roles">
        <el-radio-group v-model="admin.roles">
          <el-radio v-for="role in roles" :label="role.id" :key="role.id">{{role.name}}</el-radio>
        </el-radio-group>
      </el-form-item>
        ...
        this.$refs.admin.validate((valid) => {
          if (valid) {
            ...
            var arr = this.roles.filter(item=>{
              return item.id == this.admin.roles
            })

            const fn = id === CREATE ? createAdminApi({...this.admin, roles: arr}) :
             updateAdminApi({...this.admin, roles: arr}).then(()=>{
               localStorage.setItem('userInfo1', JSON.stringify(this.admin));
             })
            fn.then(() => {
      ...
      getRoleListApi({pageSize: 0, menu: false}).then(({list}) => {
        ...
        if (id !== CREATE) {
          getAdminApi(id).then(admin => {
            admin.roles = admin.roles.length>=1? admin.roles[0].id:null
            this.admin = admin
           
          })
        } else {
          this.rules.password.push({required: true, message: '请输入密码...', trigger: 'blur'})
          this.admin.roles = this.roles[0].id
        }
      })
    },
  }
