<template>
  <div class="content_data">
    <Breadcrumb/>
    <el-button type="primary" icon="el-icon-plus" @click="$router.push({name: 'adminEdit', params: {id: CREATE}})" :disabled="disabled">添加管理员</el-button>
    <TablePage>
      <el-table :data="data.list" v-loading="data.loading" element-loading-text="拼命加载中...">
        ...
        <el-table-column label="状态" min-width="140" fixed="right">
          <template slot-scope="scope">
            <el-switch :disabled="disabled" v-model="scope.row.status" active-color="#13ce66" inactive-color="#ff4949" active-text="启用" inactive-text="禁用" :active-value="1" :inactive-value="2" @change="statusChange(scope.row)"/>
          </template>
        </el-table-column>
