<template>
  <div class="content_data">
    ...
    <div v-loading="loading" element-loading-text="拼命加载中...">
      <el-tree :data="data" :props="options" node-key="id" :render-content="renderContent" :disabled="disabled"/>
    </div>
        ...
        if(!this.disabled) {
          ...
          <div class="tree_data">
            ...
            <div class="tree_content">
              ...
              <span style="width:20%">{data.tips}</span>
              <span style="width:40%;text-align:right;">
                <el-button plain={true}  type="danger" disabled size="mini" on-click={(e) => this.toggle(data, e)}>{['启用', '禁用'][data.status]}</el-button>
                <el-button plain={true} type="primary" disabled size="mini" on-click={(e) => this.edit(data, e)}>修改</el-button>
                <el-button plain={true} type="primary" disabled size="mini" on-click={(e) => this.create(data, e)}>创建</el-button>
                <el-button plain={true} type="danger" disabled size="mini" on-click={(e) => this.del(data, store, e)}>删除</el-button>
              </span>
            </div>
          </div>)
        }
